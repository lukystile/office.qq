...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public function saveMetaFields($insert)
    {
        ...
        if( !empty($meta) ) {
            ...
            if( !$insert ) {
                ...
                Yii::$app->db->createCommand()->batchInsert(UserMeta::tableName(), ['user_id', 'meta_key', 'meta_value'], $this->meta)->execute();
            }
        }
    }
    ...
    {
        $result = false;
        $meta_field = UserMeta::find()->where(['user_id' => $this->id, 'meta_key' => $key])->one();
    ...
    {
        return static::findOne()->where(['role' => $role]);
    }
}
